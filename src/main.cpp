...
#if defined(NDEBUG)
# error "Goshencoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2e20811b6d08c404eb01c8a0d2fc7c4a582d8933f7d87a4711194bf72424a74e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Goshencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Goshencoin Signed Message:\n";
...
{
    // Goshencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525600); // Goshencoin: 525k blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 12 * 60 * 60; // Goshencoin: 12 hours
static const int64 nTargetSpacing = 60; // Goshencoin: 1 minute blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Goshencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Goshencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2e20811b6d08c404eb01c8a0d2fc7c4a582d8933f7d87a4711194bf72424a74e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "eugene";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("375363710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1488118862;
        ...
        assert(block.hashMerkleRoot == uint256("0xbe8b469326cdc7a7c3f63bd0dca2cf753b84b2869f21baee1e7394081d47885e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb8, 0xdb }; // Goshencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GoshencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoshencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoshencoinMiner(CWallet *pwallet)
{
    printf("GoshencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goshencoin-miner");
    ...
    try { loop {
        ...
        printf("Running GoshencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoshencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoshencoinMiner, pwallet));
}
